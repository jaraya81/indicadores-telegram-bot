...
public class PLAY {
    ...
    public PLAY exec(Update update) throws TelegramException {
        if (CommandUtil.check(update, Command.PLAY)) {
            ...
            if (User.exist(user) && !StateUtil.isBanned(user) && !StateUtil.is(user, State.PLAY)) {
                ...
                userRepo.save(user);
                SendResponse sendResponse = bot.execute(new SendMessage(update.message().chat().id(), msg.msg(Msg.USER_PLAY, lang))
