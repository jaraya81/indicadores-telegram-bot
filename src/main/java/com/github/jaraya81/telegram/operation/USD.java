...
public class USD {
    ...
    public USD exec(Update update) throws TelegramException {
        if (CommandUtil.check(update, Command.GET_USD)) {
            ...
            if (User.exist(user) && !StateUtil.isBanned(user) && StateUtil.is(user, State.PLAY)) {
                log.info(Command.GET_USD.name() + " :: " + update.message().from().id() + " :: " + update.message().from().username());
                SendResponse sendResponse = bot.execute(new SendMessage(update.message().chat().id(), msg.msg(Msg.USD_RESULT, lang) + getUSD())
