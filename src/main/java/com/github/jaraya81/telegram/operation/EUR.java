...
public class EUR {
    ...
    public EUR exec(Update update) throws TelegramException {
        if (CommandUtil.check(update, Command.GET_EUR)) {
            ...
            if (User.exist(user) && !StateUtil.isBanned(user) && StateUtil.is(user, State.PLAY)) {
                log.info(Command.GET_EUR.name() + " :: " + update.message().from().id() + " :: " + update.message().from().username());
                SendResponse sendResponse = bot.execute(new SendMessage(update.message().chat().id(), msg.msg(Msg.EUR_RESULT, lang) + getEUR())
